...
// Hilfsfunktion f¸r asinh und asin: u+iv := arsinh(x+iy). Liefert cl_C_R(u,v).
...
//  = log(u+v) mod 2 pi i, und beider Imagin‰rteil ist > -pi und <= pi.)
	...
	// Da z=x+iy weder reell noch rein imagin‰r ist, ist auch
	// w := z/(1+sqrt(1+z^2)) weder reell noch rein imagin‰r.
	// (Beweis: Sollte sqrt(1+z^2) rationalen Real- und Imagin‰rteil haben,
	// so auch z, also auch w, und die Formel z = 2w/(1-w^2) zeigt, daﬂ dann
	// z reell oder rein imagin‰r sein m¸ﬂte. Also hat sqrt(1+z^2) ein
	// Float als Real- oder Imagin‰rteil, das Betragsquadrat des Nenners
	// ist also ein Float, und da Real- und Imagin‰rteil von z /=0 sind,
	// sind Real- und Imagin‰rteil von w Floats.)
	// Daher hat dann atanh(...) Floats als Realteil u und Imagin‰rteil v.
 {	DeclareType(cl_C,w);
